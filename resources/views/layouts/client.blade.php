...
<head>
    ...
    <style>
        body, .navbar-nav, .sidebar, .topbar, h1,h2,h3,h4,h5,h6,span,p { font-family: 'Poppins', sans-serif; }
        .sidebar { background-color: #2E7D32 !important; }
        .sidebar .nav-link, .sidebar .sidebar-heading, .sidebar .sidebar-brand-text, .sidebar .sidebar-brand-icon { color: #fff !important; }
        .sidebar .nav-item.active .nav-link, .sidebar .nav-link:hover { background-color: #1B5E20 !important; color: #fff !important; }
        .sidebar .sidebar-brand { background-color: #1B5E20 !important; color: #fff !important; }
        .sidebar .sidebar-brand:hover { background-color: #145A16 !important; }
        .topbar { background-color: #fff !important; }
        .topbar .navbar-nav .nav-link .fa-fw { color: #2E7D32; }
        .badge-counter { background-color: #ff6b6b !important; }
    </style>
</head>
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow-sm" style="border-bottom: 3px solid #2E7D32;">
                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <!-- Alerts Dropdown - Upcoming Assessments -->
                        <li class="nav-item dropdown no-arrow mx-1">
                            ...
                            <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="alertsDropdown">
                                <h6 class="dropdown-header">Upcoming Assessments</h6>
                                ...
                                @forelse($recentAssessments as $assessment)
                                    <a class="dropdown-item d-flex align-items-center" href="{{ route('client.schedules.index') }}">
                                        ...
                                        <div>
                                            <div class="small text-gray-500">{{ $assessment->schedule_date->format('M d, Y') }}</div>
                                            <span class="font-weight-bold">{{ $assessment->subject->code }}: {{ $assessment->title }}</span>
                                        </div>
                                    </a>
                                @empty
                                    ...
                                    <a class="dropdown-item d-flex align-items-center" href="{{ route('client.messages.index') }}">
                                        <div class="dropdown-list-image mr-3">
                                            <div class="icon-circle bg-primary">
                                                <span class="text-white font-weight-bold">{{ strtoupper(substr($message->sender->name,0,1)) }}</span>
                                            </div>
                                        </div>
                                        <div class="{{ $message->is_read ? 'font-weight-normal' : 'font-weight-bold' }}">
                                            <div class="text-truncate">{{ \Illuminate\Support\Str::limit($message->content, 30) }}</div>
                                            <div class="small text-gray-500">{{ $message->sender->name }} · {{ $message->created_at->format('M d, Y') }}</div>
                                        </div>
                                    </a>
                                @empty
                                    <a class="dropdown-item d-flex align-items-center" href="#">
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <span class="mr-2 d-none d-lg-inline text-gray-800 small font-weight-medium">{{ Auth::user()->name }}</span>
